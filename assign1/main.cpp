...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  ofs.close();
  for (Course course : not_offered) {
    delete_elem_from_vector(all_courses, course);
  }
  /*
  don't remove elements from a data structure while you are iterating over it
  for (Course course : all_courses) {
    if (course.quarter != "null") {
      //std::cout << "!= null" << course.title << ',' << course.number_of_units << ',' << course.quarter << '\n';
      delete_elem_from_vector(all_courses, course);
    } else {
      //std::cout << "== null" << course.title << ',' << course.number_of_units << ',' << course.quarter << '\n';
    }
